...
default|default-Werror|default-with-docs|valgrind|clang-format-check)
    ...
    # Start of recipe for dependency: jansson
    if ! (command -v dpkg-query >/dev/null 2>&1 && dpkg-query --list jansson-dev >/dev/null 2>&1) || \
           (command -v brew >/dev/null 2>&1 && brew ls --versions jansson >/dev/null 2>&1) \
    ; then
        echo ""
        BASE_PWD=${PWD}
        echo "`date`: INFO: Building prerequisite 'jansson' from Git repository..." >&2
        cd ./tmp-deps
