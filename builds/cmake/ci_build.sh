...
if ! ((command -v dpkg-query >/dev/null 2>&1 && dpkg-query --list jansson-dev >/dev/null 2>&1) || \
       (command -v brew >/dev/null 2>&1 && brew ls --versions jansson >/dev/null 2>&1)); then
    BASE_PWD=${PWD}
    cd tmp-deps
    $CI_TIME git clone --quiet --depth 1 https://github.com/akheron/jansson.git jansson
    cd jansson
    CCACHE_BASEDIR=${PWD}
    export CCACHE_BASEDIR
    git --no-pager log --oneline -n1
...
if [ -z "$DO_CLANG_FORMAT_CHECK" -a -f configure.ac ]; then
    ...
    $CI_TIME tar -xzf mqless-0.1.0.tar.gz
    cd mqless-0.1.0
fi
