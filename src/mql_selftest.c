...
// Now built only with --enable-drafts, so even stable builds are hidden behind the flag
    { "aws", NULL, true, false, "aws_test" },
    { "aws_sign", NULL, true, false, "aws_sign_test" },
...
{
    test_item_t *item;
    printf ("Running mqless selftests...\n");
    for (item = all_tests; item->testname; item++) {
        if (streq (item->testname, "private_classes"))
            continue;
        if (!item->subtest)
            item->test (verbose);
#ifdef MQL_BUILD_DRAFT_API // selftest is still in draft
        else
            mql_private_selftest (verbose, item->subtest);
#endif // MQL_BUILD_DRAFT_API
    }
...
main (int argc, char **argv)
{
    ...
    if (test) {
        printf ("Running mqless test '%s'...\n", test->testname);
        if (!test->subtest)
            test->test (verbose);
#ifdef MQL_BUILD_DRAFT_API // selftest is still in draft
        else
            mql_private_selftest (verbose, test->subtest);
#endif // MQL_BUILD_DRAFT_API
    }
    else
        test_runall (verbose);
